...
export const saveBigMacData = (state) => {
  try {
...
const convertCSV = (data) => {
  ...
  rows.map(row => {
      ...
      const singleElement = newTitles.reduce((object, curr, i) => (object[curr] = values[i], object), {});
...
export const getRandomCountry = (bigMacList, userCountry) => {
  const filteredBigMacs = bigMacList.filter((element) => element.Country !== userCountry);
